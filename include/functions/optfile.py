...
def handle_operateFile(instance, loaded_recv, user: Users):
    ...
    with Session() as session:
        this_path_entry = session.get(PathStructures, file_id)

        if not this_path_entry:
            instance.respond(**{"code": -1, "msg": "no such file"})
            return

        # 判断文档总体是否被删除
        if (state := json.loads(this_path_entry.state))["code"] == "deleted":
    ...
    if not specified_revision_id:  # 如果未指定 rev
        newest_revision_id: str | None = instance._getNewestFileRevisionID(file_id)

        if not newest_revision_id and req_action in [
            "read",
            "delete_rev",
            "recover_rev",
        ...
        specified_revision_id = newest_revision_id

    else:  # 如果已经指定
        # 判断是否有该 rev
        with Session() as session:
            specified_revision = session.get(FileRevisions, specified_revision_id)

            if not specified_revision:
                instance.respond(404, msg="specified revision not found")
                return

            # 判断 rev 版本是否被删除（在特别指定了 rev_id 的时候才会出现）

            if specified_revision.state == 1:
                if not view_deleted:
                    instance.respond(404, msg="specified revision not found")
                    return

    with Session() as session:  # 提前创建会话
        specified_revision = session.get(FileRevisions, specified_revision_id)

        # 重复定义是考虑到作用域问题
        this_path_entry = session.get(PathStructures, file_id)
        file_state = json.loads(this_path_entry.state)
        file_state_code = file_state["code"]
        ...
        if req_action in [
            ...
            if not instance._verifyAccess(
                user,
                req_action,
                json.loads(specified_revision.access_rules),
                json.loads(specified_revision.external_access),
            ):
                instance.respond(**instance.RES_ACCESS_DENIED)
                instance.logger.debug("权限校验失败：无权在该历史版本执行所请求的操作")
                return

            specified_revision_file_id: str = specified_revision.file_index_id
            ...
            if req_action == "read":
            ...
            elif req_action == "write":  # 该操作实际创建一个新的 revision
                do_force_write = loaded_recv["data"].get("force_write", False)

                if file_state_code != "ok":
                ...
                new_revision_id = revfunc.addRevision(
                    file_id, new_revision_file_id, parent_rev_id=specified_revision_id
                )
                ...
                with DatabaseOperator(instance._pool) as dboptr:
                    recycle_policy = Policies("recycle", *dboptr)
                    rev_delete_after_marked_time = recycle_policy[
                        "revDeleteAfterMarked"
                    ]
                    ...
                    specified_revision.state = 1
                    specified_revision.state_expire_time = (
                        time.time() + rev_delete_after_marked_time
                    )
                ...
                specified_revision.state = 0
                specified_revision.state_expire_time = 0
    ...
    with DatabaseOperator(instance._pool) as dboptr:
        ...
        dboptr[1].execute(
            "INSERT INTO path_structures \
                                (`id` , `name` , `owner` , `parent_id` , `type`, `access_rules`, `external_access`, `properties`, `state`) \
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                target_file_path_id,
                target_filename,
        ...
        ### 创建一个新的 revision
        new_revision_id = revfunc.addRevision(target_file_path_id, index_file_id)
...
def handle_getFileRevisions(instance, loaded_recv, user: Users):
    ...
    with DatabaseOperator(instance._pool) as dboptr:
        ...
        dboptr[1].execute(
            "SELECT `rev_id`, `created_time`, `state`, `state_expire_time` FROM file_revisions WHERE `path_id` = ?",
            (file_id,),
        )
        query_revisions = dboptr[1].fetchall()

    if not query_revisions:  # 若未找到 rev
        instance.respond(**instance.RES_NOT_FOUND)
        return

    with Session() as session:
        if session.get(PathStructures, file_id).type != "file":
            instance.respond(-1, msg="not a file")
            return
    ...
    sorted_revisions: list[tuple] = sorted(
        query_revisions, key=lambda i: i[1], reverse=reverse
    )
    ...
    while _i < revisions_count and _k < (
        ...
        if per_revision[2] == 1 and not view_deleted:
            continue

        this_revision_id = per_revision[0]
        with Session() as session:
            this_revision = session.get(FileRevisions, this_revision_id)

            if instance._verifyAccess(
                user,
                "read",
                json.loads(this_revision.access_rules),
                json.loads(this_revision.external_access),
            ):
                final_revisions[this_revision_id] = {
                    "created_time": this_revision.created_time,
                    "state": this_revision.state,
                    "state_expire_time": this_revision.state_expire_time,
                }
                _k += 1
